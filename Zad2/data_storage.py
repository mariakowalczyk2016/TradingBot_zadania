...
class DataStorage:
    ...
    # ZADANIE 2 <--------------------------------------------
    def close_sell(self, symbol, value):
        transaction = {
            "openedOnly": True
        }
        executes = self.command_execute('getTrades', transaction)
        for execute in executes['returnData']:
            if execute['symbol'] == symbol:
                lastposition = execute
                break
        ord = lastposition['order']
        ...
        response = self.command_execute('tradeTransaction', transaction)
        print('Close sell ', symbol, ' for ', value, ', status: ', response['status'])

    def open_sell(self, symbol, value):
        transaction = {
            "tradeTransInfo": {
                "cmd": xAPIConnector.TransactionSide.SELL,
                "order": 0,
        ...
        print('Open sell ', symbol, ' for ', value, ', status: ', response['status'])
